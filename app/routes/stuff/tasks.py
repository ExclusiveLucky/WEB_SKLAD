...
def status(request: Request, db: Session = Depends(get_db), user: str = Depends(get_user_from_cookie)):
    ...
    if user.username == "admin":
        tasks = db.query(Task).all()  # Только задачи текущего пользователя
    else:
        tasks = db.query(Task).filter(func.date(Task.created_at) == date.today()).filter(Task.user_id == user.id).all()  # Только задачи текущего пользователя

    return templates.TemplateResponse("stuff/tasks.html", {"request": request, "tasks": tasks, "user":user, "users": db.query(User).all()})
